...
network_init_para = NetworkInit(s_factor=0.0, image_para=image_para, cuda_device=device,
                                feature_channels=1, mid_channels=48, num_seq_blocks=3, 
                                num_iters=2)

initial_image = torch.ones([1,1,image_para.nx, image_para.ny], dtype=torch.float32).to(device)
