...
import math
import time
from operators.pytorch.online2d.projector import Project, BackProject, BPF
...
def test_tof2d(torch_listmode, time_resolution, pixel_size, image_grid, device):
    ...
    event_num = torch_listmode.size()[0]
    ...
    # np.save("/home/lyuli/gitpackages/test_data/bpf.npy", np.squeeze(image_bpf.cpu().numpy()))

    # proj = Project(bp, tof_value, x1l, y1l, x1r, y1r,
    #                 x2l, y2l, x2r, y2r, time_resolution, dx, dy, nx, ny, event_num, device)
    # proj_value = proj(bpf)
    # np.save("/home/lyuli/gitpackages/test_data/proj_value.npy", np.squeeze(proj_value.cpu().numpy()))
